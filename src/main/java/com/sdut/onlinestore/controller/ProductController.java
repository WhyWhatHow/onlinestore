...
import com.sdut.onlinestore.service.impl.ProductChangeVo;
...
//     notes =  "留下了不加requestBody的泪水"
    ...
    @ApiOperation(value = "根据商品id 删除一个商品", response = Integer.class)
    @GetMapping("/del/{pid}")
    public Result deleteByPid(@PathVariable("pid") String pid) {
        return service.deleteByPid(pid);
    }

    @ApiOperation(value = "根据商品id 查询上平详情一个订单", response = Integer.class)
    @GetMapping("/detail/{pid}")
    public Result detailByPid(@PathVariable("pid") String pid) {
        return service.detailByPid(pid);
    }
    ...
    @ApiOperation("批量修改商品")
    @PostMapping("/change")
    public Result changeListByPidAndState(@RequestBody ProductChangeVo vo) {
        return service.changeListByPidAndState(vo.getList(), vo.isState());
    }

    @ApiOperation("后台的模糊查询")
    @PostMapping("/list")
    public Result selectToList(@RequestBody ProductVo product) {
    ...
    public Result selectToSideShow() {
        return service.selectToSideShow();
    }
