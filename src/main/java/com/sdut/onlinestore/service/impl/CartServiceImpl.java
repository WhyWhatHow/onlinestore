...
//        TODO  逻辑存在问题,需要仔细进行校验
        ...
        if (cartItem != null) {
            ...
            cartItem.setNum(vo.getQuantity() + cartItem.getNum());
        ...
        try {
            ...
            cartItem.setQuantity(vo.getQuantity());
            System.err.println("-----------------------------------------");
            cartItem.setFee(vo.getFee());

            cartItem.setSubTotal(vo.getQuantity() * product.getPrice());
            System.err.println(cartItem);
            res = mapper.insertByVo(cartItem);
        } catch (Exception e) {
            result.setCode(500);
            TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();//
    ...
    @Override
    public Result selectAll(String uid, HttpServletRequest req) {
...
//        // todo finished
        // 获取购物车列表
        List<CartResult> list = null;
        try {
            list = mapper.selectByUid(user.getUid());
        } catch (Exception e) {
            result.setCode(500);
